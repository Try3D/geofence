...
app.post("/api/polygons/batch-json", async (req: Request, res: Response) => {
  try {
    const { points, limit: limitRaw, table: tableRaw } = req.body as {
      points: unknown;
      limit?: unknown;
    ...
    // JSON expansion: unnest points and perform set-based join with aggregation
    // Returns ALL matches per point (not just first match)
    const query = `
    ...
    const result = await pool.query<{ idx: number; matches: Array<{ osm_id: string; name: string | null }> }>(
      query,
      [lons, lats]
    );
...
app.post("/api/polygons/batch-temp", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (points.length > 100) {
      await client.query("CREATE INDEX idx_batch_points_geom ON batch_points USING GIST(geom)");
    }
