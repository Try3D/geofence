...
const LEVELS: SimplificationLevel[] = [
...
async function getVertexStats(table: string): Promise<VertexStats> {
...
async function main() {
  ...
  const levelResults: Array<{
    ...
    resultMap: Map<number, Set<string>>;
    elapsed: number;
    vertStats: VertexStats;
  }> = [];
  let originalMap: Map<number, Set<string>> | null = null;
  let originalLatency: number | null = null;

  for (const level of LEVELS) {
    process.stdout.write(`  ${level.name.padEnd(14)} ... `);

    let vertStats: VertexStats;
    try {
