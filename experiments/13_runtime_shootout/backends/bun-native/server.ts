...
// postgres (porsager): binary protocol, lower overhead than pg, Bun-native compatible
...
const server = Bun.serve({
  ...
  async fetch(req: Request) {
    ...
    if (req.method === "POST" && pathname === "/exp/13/bun-native") {
      ...
      return Response.json({ count: points.length, results: Object.values(grouped) });
    }
...
console.log(`Bun native (Bun.serve + postgres) listening on http://localhost:${server.port}`);
